...
(defmacro define-keys! (keymap &rest args)
  "Define keybindings in KEYMAP using Doom's `map!'.
See `+parse-key-groups' for the input format."
  (declare (indent 1))
  (let ((groups (+parse-key-groups args)))
    `(progn
...
(defmacro define-transient! (transient-name docstring &rest args)
...
(defmacro define-keys-and-transient! (keymap transient-name docstring &rest args)
